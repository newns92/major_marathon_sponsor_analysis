...
marathons %>%
  ...
  ggplot(aes(marathon,time)) + 
    ...
    coord_cartesian(ylim=c(as.POSIXct(paste(Sys.Date()-1,"02:00:00")),as.POSIXct(paste(Sys.Date()-1,"03:35:00"))))
...
marathons %>%
  ggplot(aes(marathon,time)) + 
    ...
    coord_cartesian(ylim=c(as.POSIXct(paste(Sys.Date()-1,"02:00:00")),as.POSIXct(paste(Sys.Date()-1,"03:00:00"))))
```
...
marathons %>%
  ...
  #geom_line(aes(group=gender,colour=gender)) 
    facet_grid(~gender) 
    #+  geom_hline(yintercept = as.POSIXct(paste(Sys.Date()-1,"02:17:00")))
  #+  geom_smooth()
```
...
marathons %>%
 # filter(marathon %in% c("NYC","London","Tokyo")) %>%
  ...
  #geom_smooth()
  facet_wrap(~marathon) +#, scales = "free_x") + 
  coord_cartesian(xlim=c(2007,2017),ylim = c(as.POSIXct(paste(Sys.Date()-1,"02:00:00")),as.POSIXct(paste(Sys.Date()-1,"02:25:00"))))
```