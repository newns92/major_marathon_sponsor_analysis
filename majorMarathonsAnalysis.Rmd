...
The [World Marathon Majors](https://www.worldmarathonmajors.com/) is a collection of six annual marathons: Tokyo, New York, Boston, London, Berlin, Chicago. And in the requisite??? years, includes the World Championship (held every 2 years by the IAAF) and the Olympic Marathon. The races are run as a series and the male and female athlete who score the most points from the qualifying races are crowned the champions of the series (defined as the points from a max of two of the qualifying races). For us non-elites, we can quality as a Six Star Finisher, wherein after successfully completing all six of the World Marathon Majors, would recieve an official certificate and Six Star Finisher Medal.

### EDA

After scraping Wikipedia for the past winners of the races and cleaning up the data, I had a [nice data file](https://github.com/newns92/major_marathon_sponsor_analysis/blob/master/marathons.R) consisting of, for each race, the marathon, the winner, the year, the winner's gender, and the winner's time. A quick first plot to look at could be the count of WMM wins by country.

```{r wins by country, echo=FALSE, message=F, warning=F}
marathons %>%
...
The common knowledge that Eastern Africa (i.e. Kenya and Ethiopa) are dominanting distance running is clearly displayed in the plot, followed by the United States, Germany, and the UK. It's pretty easy to see why, after Kenya and Ethiopia, these three countires have the most victores, as five of the six marathons tkae place in these countires. But when did this domination by the East Africans begin?
...
marathons %>% 
  ...
  ggplot(aes(x=year,color=country)) +#,group=country,col=country)) +
   ...
   # geom_point(aes(1967,37),size=3,color="purple") +  # bowerman = jogging
    geom_point(aes(1971,49),size=3,color="brown") +
    geom_text(aes(x=1956, label="Frank Shorter (1972)", y=50), colour="brown", angle=0, size=3) +
    #geom_vline(xintercept=1984) + 
    geom_point(aes(1984,90),size=3,color="brown") +
    geom_text(aes(x=1968, label="Joan Benoit (1984)", y=92), colour="brown", angle=0, size=3) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
With the incredibly imporant caveat that Boston was the only WMM that existed from its initial running in 1897 until 1970 with the first running of the New York Marathon, only those in close proximity (i.e. US and Canada) saw their counts climb. which makes this chart pretty useless unless we cut down the time period we're looking at. But, before leaving this chart completely, we can note that German and UK wins started picking up soon after first couple of their respective marathons (chicago = 1977, berlin = 1974, london = 1981, ). We can also note that, along with the estalishof ment of NYC in 70, th enumber of us winners starts to climb thans to the inclusion of women in Boston + NYC andand jsubt barely predates  Frank Shorter's win @ Olyumpics which is credidted as starting therunning bom of th e70sin america,  culminting with Benoit.

https://www.runnersworld.com/running-times-info/how-the-running-boom-began 
...
marathons %>% 
  ...
  ggplot(aes(x=year,color=country)) +#,group=country,col=country)) +
    ...
    scale_color_brewer(palette = "Dark2") +
    geom_point(aes(1984,90),size=3,color="brown") +
    geom_text(aes(x=1978, label="Joan Benoit (1984)", y=95), colour="brown", angle=0, size=3) +
    geom_point(aes(2008,35),size=3,color="#0d98ba") +
    geom_text(aes(x=2008,y=43,label="Last UK Winner (2008)"), colour="black", angle=0,
              text=element_text(size=.25)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
    coord_cartesian(xlim=c(1970,2017),ylim=c(0,150)) + 
    xlab("Country") + 
    ylab("Frequency") +
    ggtitle("Total Major Marathon Wins by Country")
```

Kenya's Rise starting in mid-90's, Ethiopia in early 2000's, Plateaus from other top countires, UK drough in 08 onways after 7 wins from Radcliffee over a 6 year span starting in 2002, gmernay having onyl 2 wins in past 20 yers, both in 08'
...
marathons %>%
  filter(!is.na(time)) %>%
  ggplot(aes(time)) + 
  geom_histogram(bins=125,na.rm=T) + 
...
marathons %>%
  ...
  #stat_density(aes(x=time,y=..scaled..,group=gender,fill=gender),color="black", alpha=.5) + 
  #geom_histogram(aes(fill=marathon),bins=15) +
  #scale_fill_brewer(palette = "Set2") +
  scale_fill_manual("Gender", values = c("Male" = "blue", "Female" = "Red")) +
  facet_wrap(~marathon) + 
